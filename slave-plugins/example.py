...
class ReceiveMessage(RPCPlugin, NotifierPlugin):
    name = 'message'
    description = 'Send a message to the user desktop'
    default_enabled = True
    capabilities = RPCPlugin.capabilities + NotifierPlugin.capabilities
    
    @log('Message received from master', logging.INFO)
    def rpc(self, message):
        self.notify(title=_('Mensaje recibido:'), text=message, 
